...
function getS3Directory(input_data) {
    ...
    var directory = `/${user}/${year}/${month}/${day}`
...
async function pushToS3(bucket_name, file_path, data) {
    var params = {
        Bucket: bucket_name,
        Key: file_path,
        Body: Buffer.from(new Uint8Array(JSON.stringify(data)))
    }

    output = await s3.putItem(params).promise()
}
...
exports.lambdaHandler = async (event, context) => {
    ...
    try {
        ...
        directory = getS3Directory(item_params)
        output = pushToS3(S3BucketName, directory, item_params)
